...
function renderPersistence(initialIndex: number, isPlaying: boolean, pageNumber = 1) {
  return renderHook(
    ({ index, playing, page }: { index: number; playing: boolean; page: number | undefined }) =>
      useProgressPersistence('doc-1', index, 10, playing, page),
    { initialProps: { index: initialIndex, playing: isPlaying, page: pageNumber } },
  );
}
...
describe('useProgressPersistence', () => {
  ...
  it('does not save a rapid second change immediately while still playing (throttled)', async () => {
    ...
    rerender({ index: 1, playing: true, page: 1 });
  ...
  it('saves immediately when not playing (manual navigation, restart, or pausing)', async () => {
    ...
    rerender({ index: 1, playing: false, page: 1 });
  ...
  it('does nothing without a documentId', async () => {
    renderHook(() => useProgressPersistence(null, 3, 10, false, 1));

    expect(await loadProgress('doc-1')).toBeUndefined();
  });

  it('saves the current page number alongside the token index', async () => {
    renderPersistence(0, false, 4);

    expect((await loadProgress('doc-1'))?.currentPageNumber).toBe(4);
  });

  it('saves an undefined page number as-is for a legacy document with no page data', async () => {
    renderHook(
      ({ page }: { page: number | undefined }) => useProgressPersistence('doc-1', 0, 10, false, page),
      { initialProps: { page: undefined } },
    );

    expect((await loadProgress('doc-1'))?.currentPageNumber).toBeUndefined();
  });
});
